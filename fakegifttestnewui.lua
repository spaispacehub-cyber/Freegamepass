--[[
SPAISPACEHUBZZZ fake gift v5 (Sharp UI + Static Checkmark + Gamepass Gift FIX + Old Settings)
--]]
...
-- ============================================================
--  CONFIG & NEW IDs (Dùng rbxthumb để chống lỗi tàng hình)
-- ============================================================
local Config = {
    FakeBalance   = 5000000,
...
local FONTS = {
    ...
    Num  = Enum.Font.GothamBold, -- Font siêu nét cho các con số
}
...
-- ============================================================
--  PURCHASE COMPLETED MODAL (FIXED CHECKMARK - NO ANIMATION)
-- ============================================================
local function showPurchaseDone(productInfo, recipient)
    ...
    -- Đã làm nhỏ lại và XOÁ ANIMATION (Hiện phát ra kích thước chuẩn luôn)
    local CH_SIZE = 54 
...
local function showBuyModal(productInfo, recipient)
    ...
    -- FIXED: NEW Badge sát lề phải y hệt ảnh (Xoá Free Trial)
...
-- ============================================================
--  HELPERS (FIX LỖI GAMEPASS CHẠY NHẦM TRÁI CHOP)
-- ============================================================

-- Sắp xếp mảng Fruits ưu tiên tên dài kiểm tra trước (tránh nhầm "Dark Blade" thành "blade")
local sortedFruits = {}
...
-- ============================================================
--  SETTINGS GUI (KHÔI PHỤC ĐÚNG GỐC 100%)
-- ============================================================
...
local function hookConfirmButton(btn)
    ...
    -- FIX LỖI GAMEPASS GIFT: Bỏ qua hoàn toàn nút điều hướng (Gift/Tặng Quà) để menu chọn người chơi hiện lên!
