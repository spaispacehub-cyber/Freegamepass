--[[
SPAISPACEHUBZZZ fake gift v5 (Sharp UI + Static Checkmark + Gamepass Gift FIX + Old Settings + Processing Animation)
--]]
...
-- ============================================================
--  PROCESSING PURCHASE MODAL (MỚI THÊM)
-- ============================================================
local function showProcessingModal(productInfo, recipient)
    ...
    box.Parent = gui
    corner(box, 8)
    stroke(box, Color3.fromRGB(70, 70, 70), 1, 0) -- Viền cho khung tối

    local iconImg = Instance.new("ImageLabel")
    iconImg.Size = UDim2.fromOffset(80, 80)
    iconImg.Position = UDim2.new(0.5, 0, 0, 25)
    ...
    task.spawn(function()
        local dots = {"", ".", "..", "..."}
        -- Vòng lặp chạy hiệu ứng chữ Processing Purchase... (chạy khoảng ~2.8 giây)
        for i = 0, 8 do
        ...
        -- Sau khi chờ xong sẽ mở UI Buy lớn ra
