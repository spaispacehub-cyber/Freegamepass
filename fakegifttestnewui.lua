...
SPAISPACEHUBZZZ fake gift v2 (Native UI & Assets Refreshed + Video Loader + Robux Alignment Fix)
--]]
...
-- ============================================================
--  CONFIG (Refreshed Native Colors & Assets)
-- ============================================================
local Config = {
    FakeBalance   = 5000000,
    BuyAccent     = Color3.fromRGB(38, 98, 245),   -- Native Fill Blue
    BuyAccentDark = Color3.fromRGB(26, 51, 127),   -- Native Dark Blue
    BgDark        = Color3.fromRGB(21, 23, 26),    -- Native Frame Background
    BgMid         = Color3.fromRGB(16, 17, 20),    -- Native Footer Background
    BgLight       = Color3.fromRGB(42, 45, 51),    -- Native Stroke Line Color
    TextMain      = Color3.fromRGB(255, 255, 255), -- TextWhite
    TextSub       = Color3.fromRGB(170, 173, 180), -- TextGrey
    PanelRed      = Color3.fromRGB(180, 20,  20),
    PanelRedDark  = Color3.fromRGB(30,  8,   8),
}

-- Native Asset IDs Slotted Directly
local CHECKMARK_ID   = "rbxassetid://100058837126647"
local ROBLOX_PLUS_ID = "rbxassetid://138882818765521"
local ROBUX_ID       = "rbxassetid://120619867489381"
local CLOSE_X_ID     = "rbxassetid://92232500754851"
...
local function lbl(parent, props)
    ...
    l.Font           = props.Font  or FONTS.Bold
...
local function robuxIcon(parent, size, pos, z)
    ...
    img.Image    = ROBUX_ID
    img.ImageColor3 = Config.TextMain
...
-- ============================================================
-- HÀM TẠO MÀN HÌNH TẢI: VIDEO Ở DƯỚI, THANH TIẾN TRÌNH Ở TRÊN (10 GIÂY)
-- ============================================================
local function showVideoLoadingScreen(videoAssetId, callback)
    local gui = Instance.new("ScreenGui")
    gui.Name = "Spacehub_VideoLoading"
    ...
    local container = Instance.new("Frame")
    container.Size = UDim2.fromOffset(550, 420)
    container.Position = UDim2.fromScale(0.5, 0.5)
    container.AnchorPoint = Vector2.new(0.5, 0.5)
    container.BackgroundTransparency = 1
    container.Parent = mainBg

    -- 1. THANH TIẾN TRÌNH Ở TRÊN
    local barBg = Instance.new("Frame")
    barBg.Size = UDim2.new(1, 0, 0, 24)
    barBg.Position = UDim2.fromOffset(0, 0)
    ...
    barFill.BackgroundColor3 = Config.BuyAccent
    ...
    videoFrame.Size = UDim2.new(1, 0, 1, -38)
    videoFrame.Position = UDim2.fromOffset(0, 38)
    videoFrame.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    videoFrame.BorderSizePixel = 0
    -- NOTE: Replace this default ID with the Video ID you upload to Roblox
    videoFrame.Video = videoAssetId or "rbxassetid://5608316401" 
    videoFrame.Looped = true
    videoFrame.Parent = container
    
    local c3 = Instance.new("UICorner")
    c3.CornerRadius = UDim.new(0, 10)
    c3.Parent = videoFrame

    videoFrame:Play()

    -- 3. ĐẾM THỜI GIAN CHẠY TỪ 1% ĐẾN 100% TRONG 10 GIÂY
    local duration = 10
    ...
    task.spawn(function()
        while true do
            ...
            local currentPercent = math.floor(1 + (progress * 99))
        ...
        percentText.Text = "100% Completed!"
        task.wait(0.8)
        
        videoFrame:Stop()
        gui:Destroy()
        
        -- Execute the callback function (showPurchaseDone) after video finishes
        if callback then
            callback()
        end
    end)
...
-- ============================================================
--  PURCHASE COMPLETED MODAL (Refreshed Native UI Look)
-- ============================================================
local function showPurchaseDone(productInfo, recipient)
    ...
    -- Pop-in Container Box for asset ID
    local CH_SIZE = 80
    local ch = Instance.new("Frame")
    ch.Size        = UDim2.fromOffset(0, 0) -- Handled dynamically by animation block
    ...
    chk.Image       = CHECKMARK_ID -- Updated to requested asset
    chk.ImageColor3 = Config.TextMain
    chk.ZIndex = 4
    chk.Parent = ch

    -- ORIGINAL POP-IN TIMELINE (UNCHANGED)
    ...
    -- ORIGINAL SLIDE IN ANIMATION (UNCHANGED)
...
-- ============================================================
--  BUY ITEM MODAL (Refreshed Native Style + Roblox Plus Footer)
-- ============================================================
local function showBuyModal(productInfo, recipient)
    ...
    -- Modal size shifted slightly taller to gracefully fit Roblox Plus footer layout natively
    local MW, MH = 460, 300
    local modal = Instance.new("Frame")
    modal.Size        = UDim2.fromOffset(MW, MH)
    modal.Position    = UDim2.fromScale(0.5, 0.6)
    ...
    -- Dynamic Right Balance Holder
    local rightContainer = Instance.new("Frame")
    rightContainer.Size = UDim2.fromOffset(180, 30)
    rightContainer.Position = UDim2.new(1, -24, 0, 22)
    rightContainer.AnchorPoint = Vector2.new(1, 0)
    rightContainer.BackgroundTransparency = 1
    rightContainer.Parent = modal

    -- X close (top-right of layout)
    local xBtn = Instance.new("ImageButton")
    xBtn.Position  = UDim2.new(1, 0, 0.5, 0)
    ...
    balTxt.Font     = FONTS.Bold
    balTxt.TextSize = 16
    balTxt.TextColor3 = Config.TextMain
    balTxt.Text     = fmtComma(Config.FakeBalance)
    balTxt.TextXAlignment = Enum.TextXAlignment.Right
    balTxt.ZIndex = 5
    balTxt.Parent = rightContainer
    balTxt.Size = UDim2.fromOffset(balTxt.TextBounds.X, 30)
    balTxt.Position = UDim2.new(1, -34, 0.5, 0)

    robuxIcon(rightContainer, 18, UDim2.new(1, -38 - balTxt.TextBounds.X, 0.5, 0), 5)
    rightContainer.Size = UDim2.fromOffset(38 + balTxt.TextBounds.X + 24, 30)

    -- ============================================================
    -- ROBUX ALIGNMENT FIX (Using UIListLayout)
    -- ============================================================
    ...
    -- This forces the icon and text to snap together perfectly
    local listLayout = Instance.new("UIListLayout")
    listLayout.FillDirection = Enum.FillDirection.Horizontal
    listLayout.VerticalAlignment = Enum.VerticalAlignment.Center
    listLayout.SortOrder = Enum.SortOrder.LayoutOrder
    listLayout.Padding = UDim.new(0, 5) 
    listLayout.Parent = priceRow

    local robuxIconObj = Instance.new("ImageLabel")
    robuxIconObj.BackgroundTransparency = 1
    robuxIconObj.Size = UDim2.fromOffset(18, 18) 
    robuxIconObj.Image = ROBUX_ID or "rbxassetid://120619867489381" 
    robuxIconObj.ImageColor3 = Color3.fromRGB(255, 255, 255)
    robuxIconObj.LayoutOrder = 1
    robuxIconObj.Parent = priceRow

    local priceLbl = Instance.new("TextLabel")
    priceLbl.BackgroundTransparency = 1
    priceLbl.Size = UDim2.fromOffset(150, 20)
    priceLbl.Font = FONTS.Bold
    ...
    -- Permanent item icon framing structure
    ...
    buyBtn.Text   = "Buy" -- Will be updated dynamically based on original button text if needed, kept generic here
    ...
    -- ── Roblox Plus Premium Footer Section ──
    ...
    plusIcon.Image              = ROBLOX_PLUS_ID -- Dynamic Requested Asset ID
    plusIcon.ZIndex             = 4
    plusIcon.Parent             = footer

    local footerTxt = Instance.new("TextLabel")
    footerTxt.BackgroundTransparency = 1
    footerTxt.Size             = UDim2.fromOffset(220, 52)
    ...
    trialBtn.Size             = UDim2.fromOffset(100, 52)
    trialBtn.Position         = UDim2.new(1, -16, 0, 0)
    trialBtn.AnchorPoint      = Vector2.new(1, 0)
    trialBtn.BackgroundTransparency = 1
    trialBtn.Text             = "Get free trial"
    trialBtn.Font             = FONTS.Main
    trialBtn.TextSize         = 14
    trialBtn.TextColor3       = Config.TextMain
    trialBtn.TextXAlignment   = Enum.TextXAlignment.Right
    trialBtn.ZIndex           = 4
    trialBtn.Parent           = footer

    local underline = Instance.new("Frame")
    underline.Size             = UDim2.fromOffset(72, 1)
    underline.Position         = UDim2.new(1, 0, 0.5, 8)
    underline.AnchorPoint      = Vector2.new(1, 0)
    underline.BackgroundColor3 = Config.TextMain
    underline.BorderSizePixel  = 0
    underline.ZIndex           = 5
    underline.Parent           = trialBtn

    -- ORIGINAL SLIDE IN + FILL TIMELINE (UNCHANGED)
    modal:TweenPosition(UDim2.fromScale(0.5, 0.5),
        Enum.EasingDirection.Out, Enum.EasingStyle.Back, 0.4, true)

    -- Auto-fill progress bar logic
    task.spawn(function()
        task.wait(0.45)
    ...
    -- Click logic with Video Interstitial Integration
    buyBtn.MouseButton1Click:Connect(function()
        ...
        -- After a short delay, destroy Buy UI and spawn the Video Loader
        task.delay(1, function()
            if gui and gui.Parent then gui:Destroy() end
            
            -- Call the video loading screen and pass a callback to show Purchase Done afterwards
            showVideoLoadingScreen("rbxassetid://5608316401", function()
...
-- ============================================================
--  HOOK (Logic completely retained unchanged)
-- ============================================================
local hookedBtns    = {}
local lastFruit     = nil
local lastRecipient = ""
...
print("[SPACEHUB] Loaded! UI Theme, Asset IDs, Robux Alignment Fix, and Video Loader updated. Logic locked down flawlessly.")
