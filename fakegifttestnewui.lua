--[[
SPAISPACEHUBZZZ fake gift v2 (Perfect Robux Alignment + Init Loading)
--]]
...
local function playInitLoading()
    ...
    task.wait(0.2)
    loadGui:Destroy() -- Tự động huỷ hoàn toàn sau khi xong
end

-- Chạy Loading ngay lập tức
playInitLoading()
...
-- ============================================================
--  BUY ITEM MODAL (FIXED ALIGNMENT TỰA HÌNH 3.JPG)
-- ============================================================
local function showBuyModal(productInfo, recipient)
    ...
    -- ==================================================
    --  FIX: TÁCH RIÊNG NÚT X VÀ GOM NHÓM ICON + SỐ DƯ (GÓC TRÊN PHẢI)
    -- ==================================================
    -- 1. Nút X (Bám sát lề phải)
    local xBtn = Instance.new("ImageButton")
    xBtn.Size = UDim2.fromOffset(24, 24)
    xBtn.Position = UDim2.new(1, -24, 0, 24) -- Cách lề phải 24px, lề trên 24px
    xBtn.AnchorPoint = Vector2.new(1, 0)
    xBtn.BackgroundTransparency = 1
    xBtn.Image = CLOSE_X_ID
    xBtn.ImageColor3 = Config.TextMain
    ...
    -- 2. Khung gom riêng Số Dư và Icon Robux (Bám sát bên trái nút X)
    local balGroup = Instance.new("Frame")
    balGroup.AnchorPoint = Vector2.new(1, 0)
    balGroup.Position = UDim2.new(1, -64, 0, 24) -- Cách nút X một khoảng trống
    ...
    balLayout.Padding = UDim.new(0, 6) -- Khoảng cách giữa icon robux và số (như ảnh là khít nhau)
    ...
    -- ==================================================
    --  FIX: CĂN GIỮA DỌC CHO GIÁ TIỀN BÊN DƯỚI (TRONG KHUNG)
    -- ==================================================
    ...
    layoutPrice.VerticalAlignment = Enum.VerticalAlignment.Center -- LUÔN LUÔN CĂN GIỮA DỌC
    layoutPrice.Padding = UDim.new(0, 6) -- Khoảng cách khít như bản gốc
    ...
    -- CHỨC NĂNG MUA (Đã xoá video loading đi, chuyển về gốc)
